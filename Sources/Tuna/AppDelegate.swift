...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func setupStatusItem() {
        ...
        if let button = statusItem.button {
            ...
            if let fishImage = NSImage(
                systemSymbolName: "fish.fill",
                accessibilityDescription: "Tuna Audio Controls"
            ) {
                // 设置图标为白色，保持一致性
        ...
        if let button = statusItem.button {
            ...
            if self.popover.isShown {
                ...
                if let buttonWindow = button.window {
                    ...
                    let convertedRect = button.window?.contentView?.convert(
                        adjustedRect,
                        from: nil
                    ) ?? buttonRect
    ...
    private func showPopover() {
        if let button = statusItem.button {
            ...
            if let buttonWindow = button.window {
                ...
                // 使用NSView中的convertRect来转换坐标系
                let convertedRect = button.window?.contentView?
    ...
    @objc func handlePinToggle(_ notification: Notification) {
        ...
        if isPinned {
               ...
               let popoverWindow = popover.contentViewController?.view.window
            {
                ...
                print(
                    "\u{001B}[33m[WARN]\u{001B}[0m Popover not shown, pin setting will apply when shown"
                )
            }
        } else {
            // 恢复 popover 的默认行为
    ...
    private func checkAccessibilityOnLaunchIfNeeded() {
        ...
        let options: NSDictionary =
            [kAXTrustedCheckOptionPrompt.takeUnretainedValue(): false] // 不立即显示系统对话框
