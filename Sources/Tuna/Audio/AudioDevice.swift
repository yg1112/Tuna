...
let kAudioDevicePropertyVirtualMasterVolume: AudioObjectPropertySelector = 1_886_680_930 // 'vmvc'
// 左右声道平衡属性
let kAudioDevicePropertyVirtualMasterBalance: AudioObjectPropertySelector = 1_886_680_946 // 'vmba'
// 立体声平衡属性
let kAudioDevicePropertyStereoPan: AudioObjectPropertySelector = 1_920_233_065 // 'span'
// 硬件服务虚拟主音量属性
let kAudioHardwareServiceDeviceProperty_VirtualMasterVolume: AudioObjectPropertySelector =
    1_936_880_500 // 'vmvc'
...
public struct AudioDevice: Identifiable, Hashable, Codable {
    ...
    public init?(deviceID: AudioDeviceID, isInput: Bool = false) {
        ...
        // 检查输入/输出能力
        self.hasInput = Self.hasDeviceCapability(
            deviceID: deviceID,
            scope: kAudioObjectPropertyScopeInput
        )
        self.hasOutput = Self.hasDeviceCapability(
            deviceID: deviceID,
        ...
        if !self.supportsBalanceControl, AudioObjectHasProperty(
            deviceID,
    ...
    private static func hasDeviceCapability(
        ...
        for property in volumeProperties {
            for element in elements {
                ...
                if AudioObjectHasProperty(id, &address) {
                    ...
                    print(
                        "设备 \(name) 属性 \(property) (元素: \(element)) 可设置状态: \(isSettable.boolValue), 状态码: \(status)"
                    )
                    ...
                    if status == noErr, isSettable.boolValue {
                        ...
                        if status == noErr {
                            print(
                                "使用属性 \(property) (元素: \(element)) 成功设置设备 \(name) 音量: \(newVolume)"
                            )
                            success = true
                            break
                        } else {
    ...
    private func setBluetoothDeviceVolume(_ volume: Float, currentBalance: Float) -> Bool {
        ...
        for selector in channelSelectors {
            ...
            if AudioObjectHasProperty(id, &leftAddress), AudioObjectHasProperty(
                id,
                &rightAddress
            ) {
    ...
    // 使用左右声道分别设置来实现平衡锁定
    private func trySetVolumeWithLockedBalanceUsingChannels(
        _ volume: Float,
        balance: Float
    ) -> Bool {
        let newVolume = max(0.0, min(1.0, volume))
    ...
    private func trySetVolumeWithLockedBalanceUsingMainVolume(
        _ volume: Float,
        balance: Float
    ) -> Bool {
        let scope = hasInput ? kAudioDevicePropertyScopeInput : kAudioDevicePropertyScopeOutput
